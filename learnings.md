...
- 자관 PILOT 1관 = 6년 NPS·"있으면 썼을 것" = 결제 의지 가설 신호
- 외부 사서 5명 인터뷰 = 2차 검증 = v2 재작성

### 사실 63: 사서 시장 정확 측정 (TAM 31,500)
- 공공 8,000·학교 사서교사 1,700·자원봉사 11,000·작은 6,830·대학 3,500·자관 500
- SAM 사서 카페 활동자 = 5,000~10,000
- SOM top 5% 결제 의지 = 1,500~3,000명
- ₩9,900 × SOM = ₩2.97~₩9.9M/월

### 사실 64: B2C UI = 사서 친화 (Cycle 60 librarian_ux 정합 강화)
- 메인 화면 = ISBN 입력·5초 .mrc·시간 절감 환산
- 일일 리포트 = "오늘 ₩76K 절감·SaaS ₩330·ROI 230:1"
- 워크플로우 위치 = 수서·정리·**현재**·배가·이용·납본
- founder 스토리 = "사서가 사서 위해 만든 SaaS"

### 사실 65: 앱스토어 = 상황별 분기 (PO 통찰)
- 웹 SaaS (Streamlit Cloud) = 앱스토어 X (불필요)
- 데스크톱 .exe (GitHub Releases) = 앱스토어 X (Sideload)
- PWA (manifest + service worker) = 앱스토어 X (즉시 핸드폰 추가)
- iOS App Store = 자동화 거절 정책 4.2.2·5.2.1·거절 거의 확실
- Google Play = 자동화 카테고리 OK·게임 매크로 = 거절
- 결제 30% 수수료 회피 = PWA + 우리 PortOne 결제

### 사실 66: 자동 클리커 = 모바일 한정 (PO 통찰 정확)
- PC 게임 자동 = 보안업체 추적·법적·BAN 위험 = 회피
- 모바일 게임 = 약관 위반·but 법적 처벌 X (회색)
- 자영업·사무·콘텐츠 = 안전 (Phase 1)
- 게임 = Phase 2·옵트인·면책
- LLM Vision (Claude Vision) = 차별화·기존 매크로 X

### 사실 67: 자동 클리커 시장 (LLM Vision 분석 비용)
- 1초 캡처·LLM Vision 호출 = 시간당 $10·일일 $3/월 = ₩4K
- 변화 감지 시만 호출 = 1/100 절감 ($0.1/시간)
- BYOK 옵션 = 마진 100%·우리 LLM = 마진 80%
- 사용자 1,000명 × ₩9,900 - 비용 = 마진 75%

### 사실 68: 진지 결정 (PO founder fit 정합)
- 사서 B2C = founder fit 강력·시장 작음·1순위 진행
- 자동 클리커 = 시장 100배·founder fit 약함·Phase 0 박제
- **권고**: 사서 B2C 1개월 시작·자동 클리커 = 결과 후 결정
- 60 사이클 코드 = 즉시 활성 (Streamlit Cloud + Supabase Auth + PortOne)

## [2026-05-06·Cycle 68] B2C 전환 + Supabase 부활 + 자동 클리커 후보

### 사실 57: PO 명시 = "수익 = 1순위·뭐든 OK·결제권자=결제자 일치"
- 도서관 B2B = 8/8 페르소나 중 1개 (P8 도서관장)만 결제 권한
- B2C 사서 본인 = "몰래 쓰기" = 결제 자유
- ₩9,900/월 = 시급 ₩20K × 6분/권 × 100권 = ₩200K 절감 = 20:1 ROI

### 사실 58: Supabase 부활 (B2C 한정·ADR 0047 부분 supersede)
- ADR 0047 = 도서관 B2B = Supabase 미도입 유지 (PIPA·CSAP·도서관 RFP)
- ADR 0050 = B2C 사서 = Supabase Auth 부활 (50K MAU 무료·결재 X)
- 자관 .mrc = 사서 컴퓨터 (헌법 §14 유지·invariant 12)
- 사서 PII만 = Supabase = privacy-policy v2 6수신자

### 사실 59: "몰래 쓰기" 메시지 = 사서 결제 의지 ↑
- 학교운영위·자치구 결재 = 부담·시간·정치
- 본인 카드 = 즉시·세금계산서 X (B2C)
- "혼자 시간 절감·운영위 모름" = 매력적
- ₩9,900/월 = 사서 본인 결제 가능 (top 5%~)

### 사실 60: 자동 클리커 SaaS = 후보 아이디어·LLM 의사결정 차별화
- 기존 자동 클리커 = 좌표만·예외 처리 0
- 우리 = LLM 의사결정·다중 작업 chain·자동 복구
- 타겟 = 자영업·비전공·콘텐츠 (TAM 한국 1,000만)
- 위치: `후보_아이디어/auto-clicker-saas/`·Phase 0 시드 박제

### 사실 61: 1인 SaaS 동시 진행 위험 (외부 901 정합)
- kormarc-auto B2C + 자동 클리커 = 2 SaaS 동시 = agent pace inflation
- 우선순위: 1) kormarc-auto B2C 인터뷰 (1주) → 2) Supabase Auth 통합 → 3) 자동 클리커 PoC
- "조금씩" = PO 명시·자동 클리커 = 박제만·진행 = kormarc-auto 인터뷰 후

## [2026-05-06·Cycle 67] 사업성 1순위 - 인터뷰 = 60 사이클 가치 검증

### 사실 52: PO 명시 (영구) = "사업성이 1순위·없는 게 가장 큰 문제"
- 60 사이클 = 코드 100%·사업성 검증 0%
- 외부 901 진단·Cycle 61 정직 진단·Cycle 67 "다른 거" = 모두 동일 결론
- 적용: 코드 추가 STOP·인터뷰 5명 = 1순위·1주 작업

### 사실 53: 인터뷰 = 60 사이클의 진짜 가치 활성
- 1주 인터뷰 = 사업성 X·O 답
- 코드 N 사이클 추가 = 답 X (외부 901 = productive avoidance)
- 결과:
  - ≥ 3.5 = 외부 5관 PILOT·계속
  - 2.5~3.5 = 5명 추가 인터뷰
  - < 2.5 = MarcEdit 모델 + 다른 도메인 검토

### 사실 54: Mom Test 5문항 = 의견·미래 X·과거 실 행동
- Q1 페인 (마지막 답답한 순간 구체적)
- Q2 현재 도구 (실 사용·MarcEdit·KOLAS·엑셀)
- Q3 시간·돈 (실 데이터)
- Q4 결재선 (학교운영위·자치구·도서관장)
- Q5 결제 권한·임계 (실 결재 가능 액수)

### 사실 55: 5명 추출 5 채널 = 사서 카페·LinkedIn·KLA/KLMA·자관·개인 네트워크
- 가장 빠른 = 사서 카페 (한국도서관협회·사서끼리·1.5만 회원)
- 가장 권한 ↑ = LinkedIn 도서관장 (P8)
- 가장 베이스라인 = PO 자관 사서 1명 (편향·but 검증)

### 사실 56: 인터뷰 안 진행 = 60 사이클 추측 = 영구 답 X
- 외부 901 진단 4중 패턴 영구 재발
- 새 SaaS 시작 = 같은 패턴
- "사업성 = 모름" = 영구 = 결정 X

## [2026-05-06·Cycle 66] 비즈니스 모델 Open Core + Hosted SaaS

### 사실 47: 코드 무료 + 서비스 유료 = 검증된 모델 (Koha·FOLIO·GitLab)
- Koha 1,399 도서관 결제·ByWater Solutions = 호스팅·지원 유료
- FOLIO 100+·EBSCO·GitLab ₩7,000억 매출
- 우리 = 동일 모델·MarcEdit (₩0 영구) ≠ 우리

### 사실 48: 도서관 결제 가능성 = 자가 5%·결제 95%
- 사서 개인 (B2C) = 자가 무료 가능 (5%)
- 학교운영위 = "GitHub 무료" = 결재 X·"공식 SaaS" = 통과
- 자치구·공공 = RFP·세금계산서·CSAP = 자가 X
- 도서관장 (P8 결재권 90%) = 책임 회피 = 결제 SaaS 1순위

### 사실 49: 결제 가치 = 시간 + 법적 + 결재 통과
- 외부 API 키 = 자가 1~3일 = 결제 즉시
- AI 기능 = BYOK 미국 카드 = SaaS 통합
- 세금계산서·SLA·환불 = 도서관 RFP 의무 = 결제만
- 학교운영위·자치구 결재 자료 자동 = 결제 가치

### 사실 50: 시간 환산 = 결제 ₩30K·자가 = ₩120K 손해
- 자가 첫 셋업 5~10시간 + 매월 1시간 = 사서 시급 ₩20K × 6시간 = ₩120K
- 결제 = 5분 가입·매월 자동
- **무료 .exe보다 결제 SaaS = 100배 가치 (시간·책임)**

### 사실 51: invariant 13 후보 = Apache-2.0 영구 보장 (신뢰 시그널)
- 코드 closed 전환·license 변경 = STOP·헌법 위반
- 자가 빌드 권리 = 영구·도서관 RFP 자가 빌드 가능 명시 = 안심 시그널
- 매출 = 서비스 패키지에서만 발생 = 코드 ≠ 매출 차단점
...
- P1 작은도서관 1인 사서: intent 90·authority 10 = 격차 80
- P3 공공 계약직: intent 60·authority 5 = 결정권 0
- P8 도서관장: intent 90·authority 90 = 격차 0 (가장 큰 PMF)
- 적용: 영업 = 결제 권한자 (P8 도서관장·KLMA) 우선·사용자 (P1·P3) ↓

### 사실 22: 시간 절감 단일 메시지 = 5/8 페르소나만 정합
- P3 (월급제·시간 ≠ 동기) → "민원 ↓·오류 ↓"
- P4 (학술 RDA·MODS) → "ProQuest 대체"
- P8 (경영자 KPI) → "사서 야근 ↓ = 본인 평판 ↑"
- 적용: 8 페르소나별 5 다른 메시지·`docs/sales/persona-message-matrix-2026-05.md`

### 사실 23: 권당 200원 alt 모델 = 8/8 페르소나 적합
- 결제 권한 분리 차단 우회 (사서 본인 결제 가능)
- 1,200관 × 200권/년 × ₩200 = ₩48,000,000 = 일반과세자 임계 정확
- 적용: 사업자 등록 = 1순위 (PO-PROD-1)

### 사실 24: AEO/GEO/SEO = 3 다른 최적화·모듈 박제·발행 0건
- SEO (네이버 60-70%) = JSON-LD + OG + robots.txt
- AEO (구글 답변 박스) = Answer-first 60단어 정의문 + 통계 밀도
- GEO (LLM 답변) = llms.txt + AI 인용 모니터링 (Phase 2 활성)
- 적용: 코드 100%·발행 0% = PO 외부 작업 (도메인·랜딩) = 1주

### 사실 25: 페르소나 시뮬 ≠ 실 사서 인터뷰 (영구 invariant 11 후보)
- 56% 전환 (Part 51) = 시뮬·인터뷰 0건
- 8 ICP 깊이 분석 = 외부 자료 종합·1차 자료 X
- 적용: 페르소나 = 가설 우선순위 도구·PMF 결정 = SALES-1 인터뷰 후
...
- 학교: 사서교사 12~15% 배치 + 정규 사서교사 미배치 84% (공무직·기간제 포함 시 48~57% 배치) = 외주 + 사서 검수만
...
## 2026-04-30 오후 — Agent token 한도 + 14-Part 매뉴얼 정점 패턴

### Agent token 한도 회피 패턴 (ADR-0017)

**사실** (4-30 8 Agent 결과):

- Agent 8개 동시 background launch 시 4건은 정상 완료 (16~120K tokens·5~10분)
- 나머지 4건은 token 한도 도달 (KST 07:40 회복 알림)
- 메인 Claude 직접 작성 = 한도 회피·즉시 완성·일관성 ↑
- 깊이 ↓ (Agent 5,000~22,000자 vs 직접 150~200줄) but 5월 PILOT 임박 정합

**적용 기준**:
- 외부 웹 조사 필수 → Agent (예: Part 7 타관 KORMARC 22K자)
- 메인 Claude 컨텍스트 활용 가능 → 직접 (예: Part 11 자관 보편성)
- 시간 임박 (5월 PILOT·KLA 5.31) → 직접 우선

### 14-Part 매뉴얼 종합 정합

**사실**:

- Part 1~6: 4-Part 매뉴얼 (113,500자) + Part 5 도구 + Part 6 Claude Code 운영
- Part 7~10: Agent 자율 조사 (4건·외부 웹 광범위)
- Part 11~14: 직접 작성 (4건·메인 컨텍스트 활용)
- 총 14건 = 한국 도서관 SaaS 시장 종합 정합

**영업 활용**:
- KLA 5.31 발표: Part 7·11·12·13 정량 직접 인용
- 도서관저널 7월호 기고: Part 12 학술 인용 14 출처
- 사서교육원 강의: Part 9 사서 커뮤니티 + Part 10 페인 + Part 12 학술
- 미국 동아시아 진출 (§33): Part 14 11관·605,000권

### 매출 영향 — VERY HIGH ★★★

- 14-Part 매뉴얼 = PO·cloud routine·외부 사서 모두 종합 참조
- 영업 정량 학술·정부·KLA·도서관저널 14 출처 직접 인용 가능
- 5년 누적 20~23억원 SOM (Part 13)
- 미국 동아시아 605,000권 (Part 14) 진출 정량
- 한국 사서 30,000명 × 본업 18~30% 환원 = 연 250~400억원 절감 (Part 12)
