...
> **대상**: 자관 「○○도서관」 사서 B·사서 C·사서 A(종합)·사서 D (3순위 ICP)
...
```powershell
kormarc-auto inspect --image data/shelf_2026-05.jpg --library PILOT_자관
```

- 책장 사진 → EasyOCR → 자관 DB 대조
- 오배가·미등록·중복 자동 검출
- 사서 D·사서 B 사서 결정 보조 (사서 책임 영역)
...
```powershell
kormarc-auto report monthly --year 2026 --month 4 --library PILOT_자관
```
...
| ☐ | 액션 |
|---|---|
| ☐ | 4명 각자 `kormarc-auto pilot-collect --persona general --library PILOT_자관` |
