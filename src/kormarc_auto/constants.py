...
_PAYMENT_INFO_DEFAULT: Final = "/pricing.html"


def get_payment_info_url() -> str:
    """가격 페이지 URL — 환경변수 우선 (외부 노출 시 절대 URL 권장)."""
    import os

    return os.getenv("KORMARC_PAYMENT_URL", _PAYMENT_INFO_DEFAULT)


PAYMENT_INFO_URL: Final = _PAYMENT_INFO_DEFAULT  # 호환성 유지 (런타임은 get_payment_info_url 권장)
